...
class FaceImageIter(io.DataIter):
    ...
    def __init__(self, batch_size, data_shape,
                 ...
                 images_filter = 0,
                 data_name='data', 
                 label_name='softmax_label', skip_hardlabel=True, **kwargs):
        ...
        if path_imgrec:
            ...
            self.skip_hardlabel = skip_hardlabel
            
            s = self.imgrec.read_idx(1)
            header, _ = recordio.unpack(s)
            try:
                label_size = header.label.shape[0]
                if label_size > 1 and self.skip_hardlabel:
                  label_size -= 1
            except:
                label_size = 1
        ...
        if label_size != 1:
          self.provide_label = [(label_name, (batch_size, label_size))]
        else:
        ...
        if self.seq is not None:
          while True:
            ...
            if self.imgrec is not None:
              ...
              label = header.label
              if self.skip_hardlabel and (not isinstance(label, float)):
                label = label[:-1]
